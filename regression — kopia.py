...
class MyLinearRegression():
    ...
    def fit_nlp(self, X: object, x_txt: object, y: object, dataframe: object):
        get_text_data = FunctionTransformer(lambda x: x[x_txt], validate=False)
        get_numeric_data = FunctionTransformer(lambda x: x[X], validate=False)

        all_variables = X + x_txt
        X_train, X_test, y_train, y_test = train_test_split(dataframe[all_variables], dataframe[y], test_size=0.2)
        print(X_train.head())
        ...
        self.r2 = process_and_join_features.score(X_test, y_test)
        print("{} model, R2 przy zmiennych {} , wynosi {}. {} obserwacji".format("NLP", X, self.r2))
